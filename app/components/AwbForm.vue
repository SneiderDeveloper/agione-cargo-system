...
const isCargoDamaged = ref(false)

const awbDetails = ref({
  pieces: '',
  weight: '',
  packagingType: [] as string[],
  warehouseLocation: '',
  shipmentEvidencePhotos: [] as File[],
  damageDescription: '',
  additionalNotes: ''
})
</script>
<template>
  ...
  <Select 
    ...
    v-model="awbDetails.packagingType"
    multiple
    :form-field-props="{
  ...
  >
    <template #actions>
      <Switch 
        v-model="isCargoDamaged"
        color="secondary"
        :label="isCargoDamaged ? 'Accepted' : 'Not accepted'"
      />
    </template>
    <Textarea
